...
> **已实现：** 7 项 ✅ | **部分实现：** 4 项 ⚠️ | **未实现：** 23 项 ❌
> **预计工期：** 8 周（5个Stage，15个Task）
>
> **⚠️ 重要更新（2026-04-19核查）：**
> - 发现 3 项特性之前误标为“已实现”，实际未实现：
>   - 多维 `operator[]` (P2128R6)
...
**❌ 待实现（P1/P2优先级）：**
- Deducing this / 显式对象参数 (P0847R7) — P1
- `auto(x)` / `auto{x}` decay-copy (P0849R8) — P1
- `static operator()` (P1169R4) — P1
- `static operator[]` (P2589R1) — P1
- `[[assume]]` 属性 (P1774R8) — P1
- 多维 `operator[]` (P2128R6) — P2 ⚠️ **核查发现：之前误标为已实现，实际未实现**
- `#warning` 预处理指令 (P2437R1) — P2 ⚠️ **核查发现：之前误标为已实现，实际未实现**
- `Z`/`z` 字面量后缀 (P0330R8) — P2 ⚠️ **核查发现：之前误标为已实现，实际未实现**
