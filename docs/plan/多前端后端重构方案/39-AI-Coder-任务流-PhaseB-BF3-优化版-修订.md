# Task B-F3：FrontendFuzzer 前端模糊测试器（优化版 - 修订）

## 修订记录

| 日期 | 修订内容 |
|------|----------|
| 2026-04-30 | 明确目录结构和 CMakeLists 集成步骤 |
...
4. 需新增 `tests/fuzz/` 目录并在根 CMakeLists.txt 中添加
...
| 新增 | `tests/fuzz/CMakeLists.txt` |
| 新增 | `tests/unit/Frontend/FrontendFuzzerTest.cpp` |
| 修改 | `CMakeLists.txt`（添加 `add_subdirectory(tests/fuzz)`） |
...
在 `add_subdirectory(tests/cpp26)` 之后添加：
...
add_subdirectory(tests/fuzz)  # ← 新增
```
...
```cmake
# FrontendFuzzer unit test (GTest)
...
1. 创建目录 `include/blocktype/Testing/`
2. 创建目录 `tests/fuzz/`
3. 创建头文件 `include/blocktype/Testing/FrontendFuzzer.h`
4. 创建实现文件 `tests/fuzz/FrontendFuzzer.cpp`
5. 创建 `tests/fuzz/CMakeLists.txt`
6. 修改根 `CMakeLists.txt` 添加 `add_subdirectory(tests/fuzz)`
7. 创建测试文件 `tests/unit/Frontend/FrontendFuzzerTest.cpp`
8. 修改 `tests/unit/Frontend/CMakeLists.txt` 添加测试目标
9. 编译验证
10. 运行测试验证
