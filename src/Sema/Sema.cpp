...
QualType Sema::InstantiateClassTemplate(llvm::StringRef TemplateName,
  ...
  // Step 5: Register the specialized record, its fields, and its methods
  // in the symbol table. TemplateInstantiator does not register declarations
  // — that is Sema's responsibility.
  registerDecl(SpecializedRecord);
  for (auto *Field : SpecializedRecord->fields()) {
    registerDecl(Field);
  }
  for (auto *Method : SpecializedRecord->methods()) {
    registerDecl(Method);
  }
...
bool Sema::isCompleteType(QualType T) const {
  ...
  // TemplateSpecializationType: check if the template specialization has been
  // instantiated as a complete type. Look for an existing specialization via
  // the instantiator.
